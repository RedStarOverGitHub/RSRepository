...
class Main:
    ...
    def __init__(self):  # 创建主界面
        ...
        menu = tk.Menu(mainMenu, tearoff=0)
        mainMenu.add_cascade(label='文件', menu=menu)
        menu.add_command(label='导入', command=self.__load)
        menu.add_command(label='保存', command=self.__save)
        menu.add_command(label='另存为', command=self.__saveAs)
        menu.add_separator()
        menu.add_command(label='退出', command=self.__closeWindow)
    ...
    def __modifyStudent(self):  # 修改某位学生的信息
        ...
        if option != None:
            try:
                ...
                def close():
                    if (stuID.get(), stuName.get(), stuGender.get(), stuChinese.get(), stuMath.get(),
                            stuEnglish.get()) != (str(ID), name, gender, *[str(i) for i in students[ID][2:]]):
                        option = msgbox.askyesnocancel('修改', '修改学生信息？')
                        if option:
                            try:
                                int(stuID.get())
                            except ValueError:
    ...
    def __addStudent(self):  # 添加学生
        def close():
            ...
            if (stuID.get(), stuName.get(), stuGender.get(), stuChinese.get(), stuMath.get(),
                    stuEnglish.get()) != ('', '', '', '', '', ''):
                option = msgbox.askyesnocancel('添加', '添加学生信息？')
                if option:
                    try:
                        int(stuID.get())
                    except ValueError:
    ...
    def __deleteStudent(self):  # 删除学生信息
        def close():
            global students
            if stuID.get() != '':
                option = msgbox.askyesnocancel('删除', '删除学生信息？')
                if option:
                    try:
